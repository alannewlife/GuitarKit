...
设备只有 UP / DOWN / OK 三个键（支持单击、双击、三击、长按）。

### 页面内操作

| 页面 | UP / DOWN | OK 单击 | OK 双击（逐层返回） |
| --- | --- | --- | --- |
| 工具箱（HUB，开机即此页） | 选工具 | 进入选中工具 | — |
| Chords · 选调 | C 大调 ↔ G 大调 | 进入该调 | 回工具箱 |
| Chords · 级数列表（I–vii°） | 移动选中行 | 查看指法 | 回选调 |
| Chords · 和弦详情 | 上一个/下一个级数（循环翻页） | 同一根音上循环拓展变体 | 回列表 |
| Tuner 调音器 | 选目标弦（即锁定检测窗口） | — | 回工具箱 |
| Capo 变调夹速查 | 改选中行数值 | 切换"夹几品/指法调" | 回工具箱 |
| Metronome 节拍器 | 速度 ±5 BPM（播放中即时生效） | 开始/停止 | 回工具箱（并停止） |

### 全局手势（任何页面生效）

| 手势 | 行为 |
| --- | --- |
| OK **三击** | 一步回到工具箱首页（离开节拍器页会停止播放） |
| OK **长按** | 立即息屏（播放中的节拍器**继续响**，可当口袋节拍器用） |
| 息屏中按任意键 | **只唤醒屏幕**，该次按键不触发任何动作（防误触） |
| 无操作 2 分钟 | 自动息屏（调音页与节拍器播放中视为正在使用，不熄屏） |
...
ES8311 麦克风 16kHz 单声道采样，每 64ms 一帧做**归一化自相关**测基频（含倍周期校正与抛物线插值），检测范围 60–500Hz，标准调弦 E2 82.41 / A2 110 / D3 146.83 / G3 196 / B3 246.94 / E4 329.63 Hz（A4=440）。选弦即锁定：检测只在选中弦预期频率 ±580 音分的窄窗内进行（排除倍/半周期误判），峰值处做 1/4 采样步长的分数滞后细化（高音弦周期短、量化误差大，细化后约 ±2 音分），结果经三帧中值滤波；偏差始终相对选中的弦计算，不会自动跳到别的弦；指针范围 ±50 音分，中央 ±5 音分绿色区显示 `IN TUNE`，超出范围时指针钉在边端（方向提示仍然有效：偏低就拧紧）。算法在 `firmware/main/tuner.c`（纯 C，可主机测试），采音任务在 `firmware/main/tuner_audio.c`。
